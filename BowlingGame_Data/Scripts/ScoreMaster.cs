...
public static class ScoreMaster {
	...
	public static List<int> ScoreFrames (List<int> rolls) {
		...
		for (int i = 1; i < rolls.Count; i += 2) {
			...
			if (rolls[i-1] + rolls[i] < 10 && rolls[i]!=-1) {				// Normal "OPEN" frame
				frames.Add (rolls [i-1] + rolls [i]);
			}

			if (rolls.Count - i <= 2) {break;}				// Ensure at least 2 look-ahead available
			...
			if (rolls[i-1] == 10) {	// STRIKE frame has just one bowl
				...
				if(rolls[i+1]<10){
					...
					if(StrikeRoll==2){
						StrikeRoll=1;
						frames.Add (10+10+ rolls [i+1]);
					}
					frames.Add (10 + rolls [i+1] + rolls[i+2]);
				}
				else if(rolls[i+1]==10 && StrikeRoll!=2){
					StrikeRoll=2;
				}
			} 
